...
export default function SundryCreditors() {
  ...
  const { data: summary } = useOutstandingSummary('creditor');
  const { data: rows = [], isLoading } = useOutstanding('creditor', filters);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Payable', value: s?.total_pending ?? 0, color: 'bg-blue-50' },
