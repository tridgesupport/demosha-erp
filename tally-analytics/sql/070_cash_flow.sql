-- ============================================================
-- tally_analytics — Cash Flow (approximate, ledger-group heuristic)
--
...
WITH cash_ledgers AS (
  SELECT name FROM tally_analytics.v_ledger_dim
...
FROM cash_lines cl
JOIN tally_analytics.v_voucher_dim vd ON vd.guid = cl.guid AND vd.is_financial
LEFT JOIN counter_lines co ON co.guid = cl.guid;

COMMENT ON VIEW tally_analytics.v_cash_flow_fact IS
...
COMMENT ON VIEW tally_analytics.v_cash_flow_summary_period IS
