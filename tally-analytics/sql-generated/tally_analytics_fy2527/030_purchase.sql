-- ============================================================
-- tally_analytics_fy2527 — Purchase analysis (mirrors 020_sales.sql)
-- ============================================================

CREATE OR REPLACE VIEW tally_analytics_fy2527.v_purchase_item_fact AS
SELECT
  vd.guid,
  vd.date,
  vd.fiscal_year,
...
JOIN tally_analytics_fy2527.v_voucher_dim vd ON vd.guid = i.guid AND vd.nature = 'Purchase'
LEFT JOIN tally_analytics_fy2527.v_item_dim id ON id.name = i.item;

COMMENT ON VIEW tally_analytics_fy2527.v_purchase_item_fact IS
...
--   REFRESH MATERIALIZED VIEW tally_analytics_fy2527.v_purchase_invoice_fact;
DROP MATERIALIZED VIEW IF EXISTS tally_analytics_fy2527.v_purchase_invoice_fact CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2527.v_purchase_invoice_fact CASCADE;
CREATE MATERIALIZED VIEW tally_analytics_fy2527.v_purchase_invoice_fact AS
WITH lines AS (
  SELECT
    a.guid,
    ld.primary_group,
...
CREATE INDEX IF NOT EXISTS v_purchase_invoice_fact_idx
  ON tally_analytics_fy2527.v_purchase_invoice_fact (fiscal_year, fiscal_quarter, month_label, vendor, purchase_channel);

COMMENT ON MATERIALIZED VIEW tally_analytics_fy2527.v_purchase_invoice_fact IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2527.v_purchase_by_vendor_period AS
...
CREATE OR REPLACE VIEW tally_analytics_fy2527.v_purchase_by_item_period AS
...
FROM tally_analytics_fy2527.v_purchase_item_fact
