-- ============================================================
-- tally_analytics_fy2325 — Balance Sheet
-- ============================================================
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_period_end AS
WITH bounds AS (
  SELECT min(date) AS mn, max(date) AS mx FROM "tallydb-fy23-25".trn_voucher
),
month_ends AS (
...
SELECT 'Month' AS period_type, period_end,
       tally_analytics_fy2325.month_label(period_end) AS period_label
FROM month_ends
UNION ALL
SELECT 'Quarter', period_end, tally_analytics_fy2325.fiscal_quarter(period_end)
FROM month_ends WHERE extract(month FROM period_end) IN (3,6,9,12)
UNION ALL
SELECT 'FY', period_end, tally_analytics_fy2325.fiscal_year(period_end)
FROM month_ends WHERE extract(month FROM period_end) = 3;

COMMENT ON VIEW tally_analytics_fy2325.v_period_end IS
...
--   REFRESH MATERIALIZED VIEW tally_analytics_fy2325.v_ledger_period_balance;
--
...
DROP MATERIALIZED VIEW IF EXISTS tally_analytics_fy2325.v_ledger_period_balance CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2325.v_ledger_period_balance CASCADE;
CREATE MATERIALIZED VIEW tally_analytics_fy2325.v_ledger_period_balance AS
WITH per_date AS (
  SELECT a.ledger, vd.date,
    SUM(CASE WHEN ld.is_debit_normal THEN -a.amount ELSE a.amount END) AS day_delta
  FROM "tallydb-fy23-25".trn_accounting a
  JOIN tally_analytics_fy2325.v_voucher_dim vd ON vd.guid = a.guid
  JOIN tally_analytics_fy2325.v_ledger_dim ld ON ld.name = a.ledger
  ...
  SELECT ld.name AS ledger,
    COALESCE(
      (SELECT SUM(CASE WHEN ld.is_debit_normal THEN -o.opening_balance ELSE o.opening_balance END)
       FROM "tallydb-fy23-25".mst_opening_bill_allocation o WHERE o.ledger = ld.name),
      CASE WHEN ld.is_debit_normal THEN -ld.opening_balance ELSE ld.opening_balance END
    ) AS opening_natural
  FROM tally_analytics_fy2325.v_ledger_dim ld
),
events AS MATERIALIZED (
  ...
  CROSS JOIN tally_analytics_fy2325.v_ledger_dim ld
),
running AS (
...
FROM running r
JOIN tally_analytics_fy2325.v_ledger_dim ld ON ld.name = r.ledger
JOIN opening o ON o.ledger = r.ledger
WHERE r.is_marker;

CREATE INDEX IF NOT EXISTS v_ledger_period_balance_idx
  ON tally_analytics_fy2325.v_ledger_period_balance (period_type, primary_group);

COMMENT ON MATERIALIZED VIEW tally_analytics_fy2325.v_ledger_period_balance IS
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_balance_sheet AS
SELECT period_type, period_end, period_label, primary_group, SUM(balance) AS balance
FROM tally_analytics_fy2325.v_ledger_period_balance
WHERE is_pnl_group = false
GROUP BY period_type, period_end, period_label, primary_group;

COMMENT ON VIEW tally_analytics_fy2325.v_balance_sheet IS
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_balance_sheet_current AS
SELECT
  primary_group,
  SUM(CASE WHEN is_debit_normal THEN -closing_balance ELSE closing_balance END) AS balance
FROM tally_analytics_fy2325.v_ledger_dim
WHERE is_pnl_group = false
GROUP BY primary_group;

COMMENT ON VIEW tally_analytics_fy2325.v_balance_sheet_current IS
