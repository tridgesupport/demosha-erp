-- ============================================================
-- tally_analytics_fy2527 — Inventory
-- ============================================================
...
COMMENT ON VIEW tally_analytics_fy2527.v_inventory_current IS
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2527.v_inventory_movement_fact AS
SELECT
...
JOIN tally_analytics_fy2527.v_voucher_dim vd ON vd.guid = i.guid AND vd.nature <> 'Receipt Note'
LEFT JOIN tally_analytics_fy2527.v_item_dim id ON id.name = i.item;

COMMENT ON VIEW tally_analytics_fy2527.v_inventory_movement_fact IS
...
--   REFRESH MATERIALIZED VIEW tally_analytics_fy2527.v_inventory_period_balance;
DROP MATERIALIZED VIEW IF EXISTS tally_analytics_fy2527.v_inventory_period_balance CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2527.v_inventory_period_balance CASCADE;
CREATE MATERIALIZED VIEW tally_analytics_fy2527.v_inventory_period_balance AS
WITH per_date AS (
  SELECT item, date, SUM(quantity) AS day_delta
  FROM tally_analytics_fy2527.v_inventory_movement_fact
  GROUP BY item, date
),
events AS MATERIALIZED (
  ...
  CROSS JOIN tally_analytics_fy2527.v_item_dim id
),
running AS (
...
FROM running r
JOIN tally_analytics_fy2527.v_item_dim id ON id.name = r.item
WHERE r.is_marker;

CREATE INDEX IF NOT EXISTS v_inventory_period_balance_idx
  ON tally_analytics_fy2527.v_inventory_period_balance (period_type, item);

COMMENT ON MATERIALIZED VIEW tally_analytics_fy2527.v_inventory_period_balance IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2527.v_inventory_by_group_period AS
...
COMMENT ON VIEW tally_analytics_fy2527.v_inventory_by_group_period IS
