...
CREATE OR REPLACE FUNCTION tally_analytics_fy2123.fiscal_quarter_number(d date)
RETURNS int
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
  SELECT CASE
      WHEN extract(month FROM d) IN (4,5,6)    THEN 1
...
CREATE OR REPLACE FUNCTION tally_analytics_fy2123.fiscal_month_number(d date)
RETURNS int
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
