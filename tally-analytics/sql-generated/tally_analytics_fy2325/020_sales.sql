-- ============================================================
-- tally_analytics_fy2325 — Sales analysis
-- ============================================================

-- ------------------------------------------------------------
-- Fact: one row per item line on a sales voucher
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_sales_item_fact AS
SELECT
  vd.guid,
  vd.date,
  vd.fiscal_year,
...
COMMENT ON VIEW tally_analytics_fy2325.v_sales_item_fact IS
...
--   REFRESH MATERIALIZED VIEW tally_analytics_fy2325.v_sales_invoice_fact;
-- ------------------------------------------------------------
DROP MATERIALIZED VIEW IF EXISTS tally_analytics_fy2325.v_sales_invoice_fact CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2325.v_sales_invoice_fact CASCADE;
CREATE MATERIALIZED VIEW tally_analytics_fy2325.v_sales_invoice_fact AS
WITH lines AS (
  SELECT
    a.guid,
    ld.primary_group,
...
SELECT
  ...
  t.sales_value,
  t.invoice_value
FROM tally_analytics_fy2325.v_voucher_dim vd
JOIN totals t ON t.guid = vd.guid
WHERE vd.nature = 'Sales';

CREATE INDEX IF NOT EXISTS v_sales_invoice_fact_idx
  ON tally_analytics_fy2325.v_sales_invoice_fact (fiscal_year, fiscal_quarter, month_label, customer, sale_channel);

COMMENT ON MATERIALIZED VIEW tally_analytics_fy2325.v_sales_invoice_fact IS
...
-- Convenience rollups
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_sales_by_customer_period AS
...
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_sales_by_item_period AS
...
FROM tally_analytics_fy2325.v_sales_item_fact
GROUP BY item, stock_group, stock_category, fiscal_year, fiscal_quarter, month_label;

CREATE OR REPLACE VIEW tally_analytics_fy2325.v_sales_by_channel_period AS
...
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_sales_by_geography_period AS
