-- ============================================================
-- tally_analytics_fy2123 — Profit & Loss Statement
--
...
DROP VIEW IF EXISTS tally_analytics_fy2123.v_profit_and_loss_current CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2123.v_profit_and_loss_summary CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2123.v_profit_and_loss CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2123.v_pnl_period_activity CASCADE;

CREATE OR REPLACE VIEW tally_analytics_fy2123.v_pnl_period_activity AS
WITH per_date AS (
...
SELECT
  tally_analytics_fy2123.month_label(pd.date)    AS month_label,
  tally_analytics_fy2123.fiscal_quarter(pd.date) AS fiscal_quarter,
  tally_analytics_fy2123.fiscal_year(pd.date)    AS fiscal_year,
  ld.primary_group,
  ld.is_direct,
...
COMMENT ON VIEW tally_analytics_fy2123.v_pnl_period_activity IS
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_profit_and_loss AS
WITH monthly AS (
  SELECT month_label, fiscal_quarter, fiscal_year, primary_group, is_direct, is_debit_normal, SUM(amount) AS amount
  FROM tally_analytics_fy2123.v_pnl_period_activity
...
COMMENT ON VIEW tally_analytics_fy2123.v_profit_and_loss IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_profit_and_loss_summary AS
SELECT
...
FROM tally_analytics_fy2123.v_profit_and_loss
GROUP BY period_type, period_label;

COMMENT ON VIEW tally_analytics_fy2123.v_profit_and_loss_summary IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_profit_and_loss_current AS
SELECT * FROM tally_analytics_fy2123.v_profit_and_loss WHERE period_type = 'FY'
  AND period_label = (SELECT max(tally_analytics_fy2123.fiscal_year(date)) FROM "tallydb-fy21-23".trn_voucher);
