-- ============================================================
-- tally_analytics_fy2123 — Cash Flow (approximate, ledger-group heuristic)
--
...
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_cash_flow_fact AS
WITH cash_ledgers AS (
  SELECT name FROM tally_analytics_fy2123.v_ledger_dim
...
FROM cash_lines cl
JOIN tally_analytics_fy2123.v_voucher_dim vd ON vd.guid = cl.guid AND vd.is_financial
LEFT JOIN counter_lines co ON co.guid = cl.guid;

COMMENT ON VIEW tally_analytics_fy2123.v_cash_flow_fact IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_cash_flow_summary_period AS
SELECT
  fiscal_year, fiscal_quarter, month_label, cash_flow_category,
  SUM(cash_natural_amount) AS net_cash_flow
FROM tally_analytics_fy2123.v_cash_flow_fact
GROUP BY fiscal_year, fiscal_quarter, month_label, cash_flow_category;

COMMENT ON VIEW tally_analytics_fy2123.v_cash_flow_summary_period IS
