...
import { fetchInventory, fetchInventoryFilters } from '@/lib/api';
...
type SortKey = 'item' | 'stock_group' | 'stock_group_parent' | 'quantity_on_hand' | 'value_on_hand';
...
export default function Inventory() {
  ...
  const { data: rows = [], isLoading, error } = useQuery<InventoryRow[]>({
    queryKey: ['inventory'],
    queryFn: () => fetchInventory() as Promise<InventoryRow[]>,
  });
  ...
  const displayed = useMemo(() => {
    ...
    return [...r].sort((a, b) => {
      ...
      if (sortKey === 'quantity_on_hand' || sortKey === 'value_on_hand') {
        av = Number(av) || 0;
        bv = Number(bv) || 0;
      }
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Latest available stock per item, as recorded in Tally
      ...
      {error ? (
        <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700">
          Failed to load inventory. The Tally analytics data source may be unavailable.
        </div>
      ) : isLoading ? (
          ...
          <div className="overflow-x-auto rounded border border-gray-200">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 text-gray-600 text-xs uppercase tracking-wide">
                <tr>
                  {([
                    ...
                    ['value_on_hand', 'Value', 'text-right'],
              ...
              <tbody className="divide-y divide-gray-100">
                {displayed.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">No items match the current filters</td></tr>
                ) : displayed.map((row) => (
                  <tr key={row.item} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 text-right font-mono text-gray-500">
                      ₹{Number(row.value_on_hand).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                    </td>
