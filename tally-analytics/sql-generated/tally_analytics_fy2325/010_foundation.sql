-- ============================================================
-- tally_analytics_fy2325 — foundation objects
-- Reads from "tallydb-fy23-25" only (for now). See README.md for
-- the Tally semantics (sign conventions, group classification,
-- voucher-nature resolution) these objects encode.
-- ============================================================

CREATE SCHEMA IF NOT EXISTS tally_analytics_fy2325;

-- ------------------------------------------------------------
-- Fiscal period helpers (India: FY runs Apr 1 -> Mar 31)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION tally_analytics_fy2325.fiscal_year(d date)
RETURNS text
...
CREATE OR REPLACE FUNCTION tally_analytics_fy2325.fiscal_quarter(d date)
RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
  SELECT 'Q' || CASE
      ...
      ELSE 4
    END || ' ' || tally_analytics_fy2325.fiscal_year(d);
$$;

CREATE OR REPLACE FUNCTION tally_analytics_fy2325.month_label(d date)
RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_group_dim AS
SELECT
  g.company,
  g.name,
  g.parent,
  ...
  g.sort_position
FROM "tallydb-fy23-25".mst_group g;

COMMENT ON VIEW tally_analytics_fy2325.v_group_dim IS
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_ledger_dim AS
SELECT
  l.company,
  l.name,
...
LEFT JOIN tally_analytics_fy2325.v_group_dim gd ON gd.name = l.parent;

COMMENT ON VIEW tally_analytics_fy2325.v_ledger_dim IS
...
-- ------------------------------------------------------------
DROP VIEW IF EXISTS tally_analytics_fy2325.v_voucher_dim CASCADE;

CREATE OR REPLACE VIEW tally_analytics_fy2325.v_voucher_dim AS
WITH vt AS (
  SELECT name, parent AS nature FROM "tallydb-fy23-25".mst_vouchertype
),
fx AS (
  -- any voucher with a foreign-currency accounting line
  SELECT DISTINCT guid FROM "tallydb-fy23-25".trn_accounting
...
SELECT
  ...
  v.guid,
  v.date,
  tally_analytics_fy2325.fiscal_year(v.date)    AS fiscal_year,
  tally_analytics_fy2325.fiscal_quarter(v.date) AS fiscal_quarter,
  tally_analytics_fy2325.month_label(v.date)    AS month_label,
  v.voucher_type,
  vt.nature,
  v.voucher_number,
  v.reference_number,
...
FROM "tallydb-fy23-25".trn_voucher v
LEFT JOIN vt ON vt.name = v.voucher_type
LEFT JOIN tally_analytics_fy2325.v_ledger_dim pl ON pl.name = v.party_name
LEFT JOIN fx ON fx.guid = v.guid;

COMMENT ON VIEW tally_analytics_fy2325.v_voucher_dim IS
