-- ============================================================
-- tally_analytics_fy2123 — Outstanding (Accounts Receivable / Payable)
--
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_bill_fact AS
SELECT
  b.ledger,
  ld.primary_group,
...
FROM "tallydb-fy21-23".trn_bill b
JOIN tally_analytics_fy2123.v_ledger_dim ld ON ld.name = b.ledger
JOIN tally_analytics_fy2123.v_voucher_dim vd ON vd.guid = b.guid
...
FROM "tallydb-fy21-23".mst_opening_bill_allocation o
JOIN tally_analytics_fy2123.v_ledger_dim ld ON ld.name = o.ledger;

COMMENT ON VIEW tally_analytics_fy2123.v_bill_fact IS
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_bill_outstanding AS
WITH agg AS (
  SELECT
  ...
  FROM tally_analytics_fy2123.v_bill_fact
...
JOIN tally_analytics_fy2123.v_ledger_dim ld ON ld.name = l.name
LEFT JOIN (
...
COMMENT ON VIEW tally_analytics_fy2123.v_ar_reconciliation_check IS
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_ap_outstanding AS
...
CREATE OR REPLACE VIEW tally_analytics_fy2123.v_ap_vendor_summary AS
WITH last_purchase AS (
  SELECT vendor, MAX(date) AS last_purchase_date
  FROM tally_analytics_fy2123.v_purchase_invoice_fact
  GROUP BY vendor
)
SELECT
  a.vendor,
...
FROM tally_analytics_fy2123.v_ap_outstanding a
LEFT JOIN last_purchase lp ON lp.vendor = a.vendor
GROUP BY a.vendor, lp.last_purchase_date;

CREATE OR REPLACE VIEW tally_analytics_fy2123.v_ap_reconciliation_check AS
SELECT
  l.name AS vendor,
  l.closing_balance,
  CASE WHEN ld.is_debit_normal THEN -l.closing_balance ELSE l.closing_balance END AS closing_balance_natural,
