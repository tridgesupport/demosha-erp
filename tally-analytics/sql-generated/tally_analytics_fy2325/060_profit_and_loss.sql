-- ============================================================
-- tally_analytics_fy2325 — Profit & Loss Statement
--
...
DROP VIEW IF EXISTS tally_analytics_fy2325.v_profit_and_loss_current CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2325.v_profit_and_loss_summary CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2325.v_profit_and_loss CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2325.v_pnl_period_activity CASCADE;

CREATE OR REPLACE VIEW tally_analytics_fy2325.v_pnl_period_activity AS
WITH per_date AS (
...
SELECT
  tally_analytics_fy2325.month_label(pd.date)    AS month_label,
  tally_analytics_fy2325.fiscal_quarter(pd.date) AS fiscal_quarter,
  tally_analytics_fy2325.fiscal_year(pd.date)    AS fiscal_year,
  ld.primary_group,
  ld.is_direct,
  ld.is_debit_normal,   -- true = Expense-like (subtract when netting), false = Income-like (add when netting)
  SUM(pd.day_amount) AS amount
FROM per_date pd
JOIN tally_analytics_fy2325.v_ledger_dim ld ON ld.name = pd.ledger
GROUP BY month_label, fiscal_quarter, fiscal_year, ld.primary_group, ld.is_direct, ld.is_debit_normal;

COMMENT ON VIEW tally_analytics_fy2325.v_pnl_period_activity IS
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_profit_and_loss AS
WITH monthly AS (
  SELECT month_label, fiscal_quarter, fiscal_year, primary_group, is_direct, is_debit_normal, SUM(amount) AS amount
  FROM tally_analytics_fy2325.v_pnl_period_activity
...
COMMENT ON VIEW tally_analytics_fy2325.v_profit_and_loss IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_profit_and_loss_summary AS
SELECT
...
FROM tally_analytics_fy2325.v_profit_and_loss
GROUP BY period_type, period_label;

COMMENT ON VIEW tally_analytics_fy2325.v_profit_and_loss_summary IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_profit_and_loss_current AS
SELECT * FROM tally_analytics_fy2325.v_profit_and_loss WHERE period_type = 'FY'
  AND period_label = (SELECT max(tally_analytics_fy2325.fiscal_year(date)) FROM "tallydb-fy23-25".trn_voucher);
