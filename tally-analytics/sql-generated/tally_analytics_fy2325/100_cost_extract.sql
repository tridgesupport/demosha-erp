-- ============================================================
-- tally_analytics_fy2325 — Cost / P&L-expense extract for Looker Studio
--
...
--   REFRESH MATERIALIZED VIEW tally_analytics_fy2325.v_cost_fact;
-- ============================================================

DROP MATERIALIZED VIEW IF EXISTS tally_analytics_fy2325.v_cost_fact CASCADE;
DROP VIEW IF EXISTS tally_analytics_fy2325.v_cost_fact CASCADE;
CREATE MATERIALIZED VIEW tally_analytics_fy2325.v_cost_fact AS
...
purchase_items AS (
  SELECT
    ...
    vd.fiscal_year,
    vd.fiscal_quarter,
    tally_analytics_fy2325.fiscal_quarter_number(vd.date) AS fiscal_quarter_number,
    tally_analytics_fy2325.fiscal_month_number(vd.date)   AS fiscal_month_number,
    vd.month_label,
    vd.voucher_type,
  ...
  JOIN tally_analytics_fy2325.v_voucher_dim vd ON vd.guid = i.guid AND vd.nature = 'Purchase'
  JOIN purchase_ledger pl ON pl.guid = i.guid
  LEFT JOIN tally_analytics_fy2325.v_item_dim id ON id.name = i.item
  LEFT JOIN tally_analytics_fy2325.v_ledger_dim vpl ON vpl.name = vd.party_name
),
other_costs AS (
  SELECT
    a.guid,
  ...
  WHERE ld.is_pnl_group = true
    ...
    AND NOT (
      ...
      AND EXISTS (SELECT 1 FROM "tallydb-fy23-25".trn_inventory i2 WHERE i2.guid = a.guid)
    )
)
...
CREATE INDEX IF NOT EXISTS v_cost_fact_idx
  ON tally_analytics_fy2325.v_cost_fact (fiscal_year, fiscal_quarter, month_label, vendor, cost_category);

COMMENT ON MATERIALIZED VIEW tally_analytics_fy2325.v_cost_fact IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_cost_by_category_period AS
...
COMMENT ON VIEW tally_analytics_fy2325.v_cost_by_category_period IS
...
CREATE OR REPLACE VIEW tally_analytics_fy2325.v_cost_by_item_period AS
...
COMMENT ON VIEW tally_analytics_fy2325.v_cost_by_item_period IS
